...
import { verifyWebhookSignature, getWebhookSecret } from '../../lib/webhookAuth'
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const subscriptionId = (body as { subscription?: { id?: string } })
    ?.subscription?.id
  const secret = subscriptionId
    ? getWebhookSecret(subscriptionId)
    : ''

  if (
    !secret ||
    !verifyWebhookSignature(signature, bodyText, secret)
  ) {
    return NextResponse.json({ error: 'unauthorized' }, { status: 403 })
  }
